...
struct Loc {
    ...
    var claudeSetupToken: String {
        switch lang {
        case .english: return "Claude setup-token"
        case .spanish: return "setup-token de Claude"
        case .french: return "setup-token Claude"
        case .german: return "Claude setup-token"
        case .japanese: return "Claude setup-token"
        case .korean: return "Claude setup-token"
        case .chineseSimplified: return "Claude setup-token"
        }
    }

    var claudeSetupTokenPlaceholder: String {
        switch lang {
        case .english: return "Paste token"
        case .spanish: return "Pegar token"
        case .french: return "Coller le token"
        case .german: return "Token einsetzen"
        case .japanese: return "トークンを貼り付け"
        case .korean: return "토큰 붙여넣기"
        case .chineseSimplified: return "粘贴令牌"
        }
    }

    var claudeSetupTokenDesc: String {
        switch lang {
        case .english:
            return "Generated by `claude setup-token`. AIPace stores it in its own Keychain item and reads it before Claude Code's Keychain."
        case .spanish:
            return "Generado por `claude setup-token`. AIPace lo guarda en su propio llavero y lo lee antes del llavero de Claude Code."
        case .french:
            return "Genere par `claude setup-token`. AIPace l'enregistre dans son propre Trousseau et le lit avant celui de Claude Code."
        case .german:
            return "Erzeugt mit `claude setup-token`. AIPace speichert ihn im eigenen Schluesselbund und liest ihn vor Claude Codes Schluesselbund."
        case .japanese:
            return "`claude setup-token` で生成します。AIPace は独自のキーチェーン項目に保存し、Claude Code のキーチェーンより先に読み込みます。"
        case .korean:
            return "`claude setup-token`으로 생성합니다. AIPace는 자체 키체인 항목에 저장하고 Claude Code 키체인보다 먼저 읽습니다."
        case .chineseSimplified:
            return "由 `claude setup-token` 生成。AIPace 会存入自己的钥匙串项，并先于 Claude Code 钥匙串读取。"
        }
    }

    var claudeSetupTokenStored: String {
        switch lang {
        case .english: return "Stored"
        ...
        case .chineseSimplified: return "已存储"
        }
    }

    var claudeSetupTokenNotStored: String {
        switch lang {
        case .english: return "Not stored"
        case .spanish: return "No guardado"
        case .french: return "Non enregistre"
        case .german: return "Nicht gespeichert"
        case .japanese: return "未保存"
        case .korean: return "저장되지 않음"
        case .chineseSimplified: return "未存储"
        }
    }

    var claudeSetupTokenSaved: String {
        switch lang {
        ...
        case .korean: return "저장됨"
        case .chineseSimplified: return "已保存"
        }
    }

    var claudeSetupTokenRemoved: String {
        switch lang {
        case .english: return "Removed"
        case .spanish: return "Eliminado"
        case .french: return "Supprime"
        case .german: return "Entfernt"
        case .japanese: return "削除しました"
        case .korean: return "삭제됨"
        case .chineseSimplified: return "已移除"
        }
    }

    var claudeSetupTokenEmpty: String {
        switch lang {
        case .english: return "Paste a token first."
        case .spanish: return "Pega un token primero."
        case .french: return "Collez d'abord un token."
        case .german: return "Fuege zuerst einen Token ein."
        case .japanese: return "先にトークンを貼り付けてください。"
        case .korean: return "먼저 토큰을 붙여넣으세요."
        case .chineseSimplified: return "请先粘贴令牌。"
        }
    }
    ...
    var remove: String {
        ...
        case .chineseSimplified: return "移除"
        }
    }
    ...
    var providersDesc: String {
        switch lang {
        case .english:
            return "Codex uses codex app-server. Claude reads credentials from file, AIPace setup-token, CLAUDE_CODE_OAUTH_TOKEN, then Claude Code Keychain."
        case .spanish:
            return "Codex usa codex app-server. Claude lee credenciales desde archivo, setup-token de AIPace, CLAUDE_CODE_OAUTH_TOKEN y luego el llavero de Claude Code."
        case .french:
            return "Codex utilise codex app-server. Claude lit les identifiants depuis le fichier, le setup-token AIPace, CLAUDE_CODE_OAUTH_TOKEN, puis le Trousseau de Claude Code."
        case .german:
            return "Codex verwendet codex app-server. Claude liest Anmeldedaten aus Datei, AIPace setup-token, CLAUDE_CODE_OAUTH_TOKEN und dann Claude Codes Schluesselbund."
        case .japanese:
            return "Codex は codex app-server を使います。Claude はファイル、AIPace setup-token、CLAUDE_CODE_OAUTH_TOKEN、Claude Code キーチェーンの順で認証情報を読み込みます。"
        case .korean:
            return "Codex는 codex app-server를 사용합니다. Claude는 파일, AIPace setup-token, CLAUDE_CODE_OAUTH_TOKEN, Claude Code 키체인 순으로 자격 증명을 읽습니다."
        case .chineseSimplified:
            return "Codex 使用 codex app-server。Claude 会依次从文件、AIPace setup-token、CLAUDE_CODE_OAUTH_TOKEN 和 Claude Code 钥匙串读取凭证。"
        }
    }
    ...
    func menuBarStatusLabel(_ status: AgentStatus) -> String? {
        ...
        case .rateLimited:
            switch lang {
            case .english: return "Wait"
            case .spanish: return "Espera"
            case .french: return "Attente"
            case .german: return "Warten"
            case .japanese: return "待機"
            case .korean: return "대기"
            case .chineseSimplified: return "等待"
            }
    ...
    func statusInstruction(_ status: AgentStatus) -> String? {
        ...
        case (.claude, .accessDenied):
            switch lang {
            case .english: return "Re-save the Claude setup-token in Settings, or allow Keychain access if macOS prompts."
            case .spanish: return "Vuelve a guardar el setup-token de Claude en Ajustes, o permite el acceso al llavero si macOS lo solicita."
            case .french: return "Enregistrez a nouveau le setup-token Claude dans les reglages, ou autorisez l'acces au Trousseau si macOS le demande."
            case .german: return "Speichere den Claude setup-token in den Einstellungen erneut oder erlaube den Schluesselbundzugriff, falls macOS fragt."
            case .japanese: return "設定で Claude setup-token を再度保存するか、macOS が求めた場合はキーチェーンアクセスを許可してください。"
            case .korean: return "설정에서 Claude setup-token을 다시 저장하거나, macOS가 요청하면 키체인 접근을 허용하세요."
            case .chineseSimplified: return "请在设置中重新保存 Claude setup-token，或在 macOS 提示时允许钥匙串访问。"
            }
        ...
        case (.claude, .rateLimited):
            if let message = status.message {
                return message
            }
            switch lang {
            case .english: return "Claude usage is rate limited. Wait a bit before refreshing again."
            case .spanish: return "El uso de Claude tiene límite de tasa. Espera un poco antes de actualizar de nuevo."
            case .french: return "L'utilisation de Claude est limitee. Patientez avant d'actualiser a nouveau."
            case .german: return "Claude-Nutzung ist rate-limitiert. Warte etwas, bevor du erneut aktualisierst."
            case .japanese: return "Claude の使用量取得がレート制限されています。少し待ってから再度更新してください。"
            case .korean: return "Claude 사용량 조회가 요청 제한 중입니다. 잠시 기다린 뒤 다시 새로고침하세요."
            case .chineseSimplified: return "Claude 用量请求已被限速。请稍等后再刷新。"
            }
